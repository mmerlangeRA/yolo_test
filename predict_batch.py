...
def pixelate_region(image:cv2.typing.MatLike, x1:int, y1:int, x2:int, y2:int, pixelation_level=10)->None:
    """
...
def split_image_in_squares(image:cv2.typing.MatLike)->List[ImageExtract]:
    ...
    squares = [
        ...
        ImageExtract(quarter_width, h//3, center[:, quarter_width:quarter_width*2]),
        ImageExtract(quarter_width*2, h//3, center[:, quarter_width*2 :quarter_width*3]),
...
def save_images(images, output_paths)->List[str]:
    failed_saved_paths=[]
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(save_image, (output_paths[i], images[i])) for i in range(len(images))]
...
def anonymize_images_array(prod_images: List[cv2.typing.MatLike], debug=False) -> List[cv2.typing.MatLike]:
    ...
    for image_index in range(len(prod_images)):
        ...
        square_index=0
        for extract_index in range(image_index * nb_extracts_per_image,(image_index+1) * nb_extracts_per_image):
            square_index+=1
            result = results[extract_index]
            extract = all_extracts[extract_index]
            boxes = result.boxes
            if debug:
                print(f"Image {image_index}, square {square_index}, nb boxes: {len(boxes)}")
            for box in boxes:
...
for batch in image_paths_batches:
    ...
    if len(failed_images_paths) > 0:
        ...
        print(f"Failed to load {len(failed_images_paths)} images: {failed_images_paths}")
    ...
    try:
        anonymize_images_array(images,debug=False)
    except Exception as e:
    ...
    if len(failed_images_paths) > 0:
        nb_failures+=len(failed_images_paths)
        print(f"Failed to save {len(failed_images_paths)} images: {failed_images_paths}")
...
if len(failed_images_processing_paths) > 0:
    print(f"Failed to process {len(failed_images_processing_paths)} images: {failed_images_processing_paths}")
if len(failed_images_saving_paths) > 0:
    print(f"Failed to save {len(failed_images_saving_paths)} images: {failed_images_saving_paths}")
